...
![微信图片_20200525215712](images/微信图片_20200525215712.png)
...
Rabbitmq提供了一种 qos 服务质量保障功能，即在**非自动确认消息**的前提下，如果一定数目的消息未被确认，不进行消费新的消息。
...
消费端的**重回队列**是为了对没有处理成功的消息，把消息重新递给 broker。但是在我们的实际生产，一般都会关闭重回队列，
...
dead-letter-exchange，当消息在一个队列中变成死信 (dead message) 之后，也就是没有被消费，它能被重新 publish 到另一个 exchange，这个 exchange 就是 dlx。
...
当某个队列中有死信时，rabbitMQ 就会自动的将这个消息重新发布到设置的 exchange（dlx） 上去，进而被路由到另一个队列，可以监听这个队列中消息做相应的处理。
...
public static void main(String[] args) throws IOException, TimeoutException {
        ...
        //交换机 和 队列 根据 路由key 进行 绑定
        channel.queueBind(queueName, exchangeName, routingKey);

        //真正死信队列的声明: dlx.exchange/queue都是由你自己命名的
